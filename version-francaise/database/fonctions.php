...
function addIntern($internData) {
    try {
        ...
        $stmt = $db->prepare('INSERT INTO users (username, password, user_type, email, full_name, department) 
                              VALUES (?, ?, ?, ?, ?, ?)');
        $username = generateUsername($internData['first_name'], $internData['last_name']);
        $fullName = $internData['first_name'] . ' ' . $internData['last_name'];
        $stmt->execute([$username, $hashedPassword, 'intern', $internData['email'], $fullName, $internData['department']]);
        $userId = $db->lastInsertId();
        
        // Add intern record
        $stmt = $db->prepare('INSERT INTO interns (user_id, first_name, last_name, email, id_card, university, 
                             department, supervisor_id, start_date, end_date) 
                             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        $stmt->execute([
            $userId,
...
function updateIntern($internId, $internData) {
    try {
        ...
        // Update user info
        $stmt = $db->prepare('UPDATE users SET email = ?, full_name = ?, department = ? WHERE id = ?');
        $fullName = $internData['first_name'] . ' ' . $internData['last_name'];
        $stmt->execute([$internData['email'], $fullName, $internData['department'], $userId]);
...
function addSupervisor($supervisorData) {
    try {
        ...
        $stmt = $db->prepare('INSERT INTO users (username, password, user_type, email, full_name, department, director_id) 
                              VALUES (?, ?, ?, ?, ?, ?, ?)');
...
function deleteDepartment($departmentId) {
    try {
        ...
        $stmt = $db->prepare('SELECT department FROM users WHERE department = (SELECT name FROM departments WHERE id = ?)');
        $stmt->execute([$departmentId]);
        if ($stmt->rowCount() > 0) {
