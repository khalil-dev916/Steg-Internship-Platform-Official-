...
function initializeDatabase() {
    ...
    // Migration columns for existing DB
    addColumnIfNotExists($db, 'users', 'department_id', 'INTEGER');
    addColumnIfNotExists($db, 'users', 'director_id', 'INTEGER');
    addColumnIfNotExists($db, 'interns', 'department_id', 'INTEGER');

    // Backfill department_id from legacy TEXT columns if present
    try {
        $db->exec('UPDATE users SET department_id = (
            SELECT id FROM departments d WHERE d.name = users.department
        ) WHERE (department_id IS NULL OR department_id = "") AND department IS NOT NULL');
    } catch (Exception $e) { }
    try {
        $db->exec('UPDATE interns SET department_id = (
            SELECT id FROM departments d WHERE d.name = interns.department
        ) WHERE (department_id IS NULL OR department_id = "") AND department IS NOT NULL');
    } catch (Exception $e) { }
    ...
    if ($stmt->fetchColumn() == 0) {
        ...
        $q->execute(['Informatique']);
        $deptId = ($r=$q->fetch(PDO::FETCH_ASSOC)) ? $r['id'] : null;
...
function checkLoginByEmail($email, $password) {
    ...
    $stmt = $db->prepare('SELECT u.*, d.name AS department FROM users u LEFT JOIN departments d ON d.id = u.department_id WHERE u.email = ?');
    $stmt->execute([$email]);
